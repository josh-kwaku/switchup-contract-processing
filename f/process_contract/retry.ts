...
export async function main(workflowId: string) {
  ...
  const res = await fetch(`${serviceUrl}/workflows/${workflowId}/retry`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });

  const body = await res.json();
  if (!body.success) throw new Error(`Retry failed (HTTP ${res.status}): ${body.error?.message ?? "unknown error"}`);
  return body.data;
}
